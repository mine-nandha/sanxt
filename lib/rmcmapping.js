...
export function extractInformation(data) {
	...
	const nodeNameRegex = /Node Name:\s*([A-Za-z0-9\s'`/-]+)/;
	const zipRegex = /->\s*([A-Za-z0-9]+)/;
	const streetAddressRegex = /Street:\s*([A-Za-z0-9\s'`/-]+)(?=\s*Locality|$)/;
	...
	const nodeName = nodeNameMatch
		? nodeNameMatch[1]
				...
				.replace(/(?<=[`'])\w/g, (char) => char.toLowerCase()) // Lowercase letters after backticks/apostrophesnp
		: "Not found";
